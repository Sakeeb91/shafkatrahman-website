...
The canonical article inventory lives in `writing-manifest.js`. It is the source of truth for article paths, dates, types, and visible tags.
...
7. Verify the article page, writing index tags and filters, homepage references, structured metadata, mobile layout, and both themes.

The shared shell injects visible article tags and `article:tag` metadata from the manifest. The writing index reads the same registry, so tag changes propagate without editing multiple tag lists.
